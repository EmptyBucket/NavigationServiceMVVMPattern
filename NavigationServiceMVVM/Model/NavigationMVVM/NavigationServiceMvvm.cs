...
{
    public sealed class NavigationServiceMvvm : INavigationService
    {
        ...
        private static volatile NavigationServiceMvvm _instance;
        private static readonly object SyncRoot = new object();
        private readonly Dictionary<string, DataPage> _pages;

        private NavigationServiceMvvm()
        {
            _historic = new List<string>();
            _pages = new Dictionary<string, DataPage>();
        }

        public static NavigationServiceMvvm Service
        {
            get 
            {
                if (_instance != null) return _instance;
                lock (SyncRoot)
                if (_instance == null) _instance = new NavigationServiceMvvm();
                return _instance;
            }
        }
    }
